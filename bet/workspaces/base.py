...
class Workspace:
        ...
        with utils.eval_mode(
            ...
            if self.cfg.use_state_prior:
                ...
                latents = self.state_prior.generate_latents(
                    ...
                    latent_sampling_strategy=getattr(
                        self.cfg, "latent_sampling_strategy", None
                    ),
                )
    ...
    @property
    def snapshot(self):
        load_dir = Path(self.cfg.load_dir or self.work_dir)
        default_snapshot = load_dir / "snapshot.pt"
        if default_snapshot.exists():
            return default_snapshot

        tagged_snapshots = sorted(load_dir.glob("snapshot_*_offset.pt"))
        if len(tagged_snapshots) == 1:
            return tagged_snapshots[0]
        return default_snapshot
