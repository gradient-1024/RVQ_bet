...
class Workspace:
    ...
    def _init_state_prior(self):
        if self.state_prior is None:  # possibly already initialized from snapshot
            ...
            self._set_state_prior_rvq_codebooks()
            if self.cfg.data_parallel:
    ...
    def _set_state_prior_rvq_codebooks(self):
        if not hasattr(self.state_prior, "set_rvq_codebook_embeddings"):
            return
        action_ae = self.action_ae.module if self.cfg.data_parallel else self.action_ae
        if not hasattr(action_ae, "get_codebook_embeddings"):
            return
        self.state_prior.set_rvq_codebook_embeddings(
            action_ae.get_codebook_embeddings(detach=True)
        )
    ...
    def run(self):
        ...
        legacy_snapshot = self.work_dir / "snapshot.pt"
        if not snapshot.exists() and snapshot != legacy_snapshot:
    ...
    @property
    def snapshot(self):
        tag = self._rvq_offset_mode_tag()
        filename = f"snapshot_{tag}.pt" if tag is not None else "snapshot.pt"
        return self.work_dir / filename

    def _rvq_offset_mode_tag(self):
        state_prior_cfg = getattr(self.cfg, "state_prior", None)
        if state_prior_cfg is None:
            return None
        rvq_num_quantizers = state_prior_cfg.get("rvq_num_quantizers", None)
        predict_offsets = state_prior_cfg.get("predict_offsets", False)
        if rvq_num_quantizers is None or not predict_offsets:
            return None
        return state_prior_cfg.get(
            "offset_mode_tag",
            state_prior_cfg.get("offset_mode", "code_independent") + "_offset",
        )
    ...
    def log_append(self, log_key, length, loss_components):
        for key, value in loss_components.items():
            self._log_component(log_key, key, length, value)
            alias_log_key = self._accuracy_metric_alias_log_key(log_key, key)
            if alias_log_key is not None:
                self._log_component(alias_log_key, key, length, value)

    def _log_component(self, log_key, key, length, value):
    ...
    @staticmethod
    def _accuracy_metric_alias_log_key(log_key, key):
        if not (key.startswith("acc_code") or key.startswith("acc3_code")):
            return None
        if log_key == "prior_train":
            return "train"
        if log_key == "prior_eval":
            return "val"
        return None
